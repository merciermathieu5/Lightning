...
(async () => {
  ...
  egal(S.SECOURS_ROSTER.length, 23, '23 joueurs dans la formation de secours (pro)');
  const barkov = S.SECOURS_ROSTER.find(j => j.nom === 'Aleksander Barkov');
  egal(barkov.salaire, 8500000, 'Salaire Barkov');
  egal(barkov.ct, 1, 'Contrat Barkov 1 an');
  egal(S.SECOURS_Y21.length, 21, '21 patineurs dans la référence Y21 (capture XtraStats du DG)');
  const barkovY21 = S.SECOURS_Y21.find(x => x.nom === 'Aleksander Barkov');
  egal(barkovY21.pts, 77, 'Barkov 77 points en Y21');
  egal(barkovY21.hits, 99, 'Barkov 99 MEÉ en Y21');
  const hanleyY21 = S.SECOURS_Y21.find(x => x.nom === 'Joel Hanley');
  egal(hanleyY21.pts, 0, 'Hanley 0 point en 10 matchs (validé par les ratios P/20 et P/G de la page)');
  ...
  egal(profilDe('Aleksander Barkov').profil, 'Elite', 'Barkov → Elite');
  egal(profilDe('Aleksander Barkov').mat, 'ELITE', 'Barkov → matrice ELITE');
  tableauEgal(profilDe('Aleksander Barkov').stats, ['shotpct','gwg','ppg','pts','pmrang'],
    'Stats évaluées Elite = tableau 20 (PCTG, GWG, PP, P, +/-)');
  egal(profilDe('Grigori Denisenko').profil, 'Elite', 'Denisenko → Elite');
  egal(profilDe('Jacob de la Rose').profil, 'Grinder', 'De la Rose → Grinder');
  tableauEgal(profilDe('Jacob de la Rose').stats, ['hits20'], 'Stats Grinder = MEÉ/20');
  egal(profilDe('Lucas Raymond').profil, 'Prospect Elite', 'Raymond (23 ans) → Prospect Elite');
  egal(profilDe('Lucas Raymond').mat, 'ELITE', 'Prospect Elite → même matrice ELITE');
  egal(profilDe('Jacob Trouba').profil, 'DEliteShutdown', 'Trouba → DEliteShutdown');
  egal(profilDe('Owen Power').profil, 'Prospect DEliteQB', 'Power (23 ans) → Prospect DEliteQB');
  egal(profilDe('Adam Parsells').profil, 'DDefensive', 'Parsells → DDefensive');
  egal(profilDe('Colby Barlow').profil, 'Junior Elite', 'Barlow (20 ans) → Junior Elite');
  egal(profilDe('Oscar Dansk').profil, 'Starter Goalie', 'Dansk (OV 84) → Starter Goalie');
  tableauEgal(profilDe('Oscar Dansk').stats, ['hs','svpct','qggp','ming'], 'Stats Starter = HS, SV%, QG/GP, MIN');
  egal(profilDe('Adin Hill').profil, 'Backup Goalie', 'Hill (OV 79) → Backup Goalie');
  tableauEgal(profilDe('Adin Hill').stats, ['mp','qggp','psv'], 'Stats Backup = MP, QG/GP, Psv');
  ...
  const four = S.fourchetteRecote(barkov, 10); // 30 ans → ModA-5 → base 65
  ...
  const jk = joueurDe('Aleksander Barkov');
  jk._profil = S.determinerProfil(jk);
  let s = S.seuilsPour(jk, 'pts', {}, {});
  tableauEgal(s.seuils, [111,98,85,73,43,-1], 'Seuils Y17 par défaut (OV 84)');
  egal(s.source, 'Y17', 'Source = Y17');
  const perso = {ELITE: {pts: {84: [100,90,80,70,40,-1]}}};
  ...
  egal(s.seuils[2], 85, 'Les autres degrés restent ceux de la matrice');
  ...
  const fixtureXtra = [
    ...
    '    [Aleksander Barkov](https://x/#Aleksander Barkov) TAMPABAY        C   82  30  45  75 200  30 1700 100 2.44',
    '    [Alex Tuch](https://x/#Alex Tuch) ANAHEIM         RW  77  27  32  59 183  16 1503 243 2.38',
    '    [* Backup_RW](https://x/#Backup_RW) TAMPABAY        LW   3   0   0   0   0   0    0   0 0.00'
  ].join('\n');
  const x = S.parseXtra(fixtureXtra, 'TAMPABAY');
  egal(x.length, 2, 'Deux patineurs TAMPABAY extraits (Anaheim exclu)');
  ...
  ok(doc.querySelector('#ficheEquipe').textContent.includes('TAMPABAY'), 'Fiche d\'équipe affichée');
  ...
  egal(masse, 77996000, 'Masse salariale des 23 pros = 77 996 000 $');
  ...
  console.log('— Divers');
  egal(S.matchsEquipe(), 0, 'Fiche TAMPABAY 0-0-0 → 0 match d\'équipe');
  ...
  if (ETAT){
    ...
    ETAT.xtra = [{nom:'Aleksander Barkov', gp:9, goals:4, assists:6, pts:10, shots:25, pim:2, mp:180, hits:8}];
    const jBarkov = S.SECOURS_ROSTER.find(x2=>x2.nom==='Aleksander Barkov');
    const prodRepli = S.productionDe(jBarkov);
  ...
  const prodY21 = S.productionDe(S.SECOURS_ROSTER.find(x2=>x2.nom==='Aleksander Barkov'));
  egal(prodY21?._modeY21, true, 'Production servie par les données Y21 intégrées');
  egal(prodY21?.pts, 77, 'Points Y21 de Barkov');
  const carteBarkov = [...doc.querySelectorAll('#progGrille .joueur-carte')]
    .find(c=>c.querySelector('.jc-nom').textContent==='Aleksander Barkov');
  ok(!!carteBarkov, 'Carte de Barkov rendue en mode Y21');
  ok(carteBarkov.textContent.includes('Mode vérification Y21'), 'Note du mode Y21 sur la carte');
  const jBk = S.ETAT.roster.find(x2=>x2.nom==='Aleksander Barkov');
  const evPtsB = S.evaluerStat('pts', S.valeurStat(jBk, prodY21, 'pts', {}), S.seuilsPour(jBk, 'pts', {}, {}).seuils);
  egal(evPtsB.statut, 'correcte', 'Barkov Correcte en points (73 < 77 ≤ 85, Elite OV 84)');
  const jRose = S.ETAT.roster.find(x2=>x2.nom==='Jacob de la Rose');
  const vHits = S.valeurStat(jRose, S.productionDe(jRose), 'hits20', {});
  proche(vHits, 3.44, 0.01, 'De la Rose 3,44 MEÉ/20 min en Y21 (246 MEÉ, 1432 min)');
  const evHits = S.evaluerStat('hits20', vHits, S.seuilsPour(jRose, 'hits20', {}, {}).seuils);
  ok(['satisfaisante','excellente','memorable'].includes(evHits.statut),
    'De la Rose atteint au moins Satisfaisante en MEÉ/20 (Grinder)');
  ok(!carteBarkov.textContent.includes('proj.'), 'Aucune projection affichée (saison complète)');
  btnY21.click();
